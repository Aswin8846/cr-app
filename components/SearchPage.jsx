...
const SearchPage = () => {
  ...
  const processImage = async (fileUri) => {
    ...
    try {
      const response = await axios.post("https://e2f6-2406-7400-bb-7d6d-fba2-a3ca-4f82-79a0.ngrok-free.app/upload", formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      ...
      if (response.status === 200) {
        ...
        const matchedImagesData = data.matched_images || [];
        const matchedImagesUrls = matchedImagesData.map((imageData) => ({
          url: `https://e2f6-2406-7400-bb-7d6d-fba2-a3ca-4f82-79a0.ngrok-free.app/matched-images/${imageData.file_name}`,
          matchRate: imageData.match_rate,
        }));
